...
```bash
# Run the full test suite (equivalent to: tester tests -s)
composer run tester
```
...
- `read(string $key)` - returns mixed (`@return mixed` in PHPDoc, no declared return type); null if missing/expired
- `lock(string $key): void` - Prevents item reading and writing; lock is released by `write()` or `remove()`
- `write(string $key, mixed $data, array $dependencies): void`
