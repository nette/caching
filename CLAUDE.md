...
Note: there is no separate coding-standard/lint command in `composer.json`. The
only defined scripts are `phpstan` (static analysis) and `tester` (runs the test
suite via `tester tests -s`).
